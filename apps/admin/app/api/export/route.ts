import { ALLOWED_EMAILS, EXCLUDED_EMAILS } from "@/lib/constants";
import { arrayToCsv, getMarket } from "@/lib/utils";
...
export async function GET(req: NextRequest) {
  ...
  if (type === "users") {
    ...
    for (const h of holdings ?? []) {
      holdingsCountMap.set(
        h.user_id,
        (holdingsCountMap.get(h.user_id) ?? 0) + 1,
      );
    }
    ...
    const tickerMap = new Map<
      string,
      { name: string; ticker: string; users: Set<string>; currency: string }
    >();
    for (const h of holdings ?? []) {
    ...
    const sorted = Array.from(tickerMap.values()).sort(
