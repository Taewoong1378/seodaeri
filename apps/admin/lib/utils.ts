...
export function isETF(name: string | null, ticker: string): boolean {
  ...
  const etfPatterns = ['ETF', 'etf', 'KODEX', 'TIGER', 'KBSTAR', 'SOL', 'ACE', 'HANARO', 'ARIRANG', 'KOSEF', 'TIMEFOLIO', 'PLUS']
  // US ETFs: 3-4 letter tickers that match known ETF patterns
  const usEtfTickers = ['SPY', 'QQQ', 'IVV', 'VOO', 'VTI', 'SCHD', 'VEA', 'IEFA', 'AGG', 'BND', 'VWO', 'VIG', 'VYM', 'IWM', 'EFA', 'GLD', 'TLT', 'XLK', 'DIA', 'ARKK', 'JEPI', 'JEPQ', 'TQQQ', 'SOXL', 'SOXS']
  return etfPatterns.some(p => name.includes(p)) || usEtfTickers.includes(ticker)
}
