...
let createMessageHandler: ((ref: React.RefObject<WebView>) => (event: any) => void) | null = null
try {
...
function AppContent() {
  ...
  useEffect(() => {
    ...
    const safetyTimer = setTimeout(() => {
      console.log('[AppContent] Safety timeout (10s) - forcing splash hide')
      hideSplash()
    }, 10000)
  ...
  const onNav = (s: { url: string; canGoBack: boolean }) => {
    try {
      const root = ['/', '/dashboard', '/login', '/onboarding'].includes(new URL(s.url).pathname)
