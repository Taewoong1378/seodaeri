...
import { Banknote, Calendar, Pencil, PiggyBank, Trash2, TrendingDown, Wallet } from 'lucide-react'
import { useSession } from 'next-auth/react'
import { useMemo, useState } from 'react'
import { getTransactionBannerByTab } from '../../../../lib/banner-data'
import { queryKeys } from '../../../../lib/query-client'
import { type AccountBalanceRecord, deleteAccountBalance } from '../../../actions/account-balance'
...
export function TransactionsClient({
  ...
  const [deleteBalanceTarget, setDeleteBalanceTarget] = useState<AccountBalanceRecord | null>(null)
  ...
  // 최신 계좌총액 계산
  const latestBalance = accountBalances.length > 0 ? accountBalances[0]?.balance || 0 : 0
      ...
      <div className="space-y-4">
        ...
        <div className="flex items-center bg-muted p-1 rounded-full border border-border">
          ...
          >
            ...
            <span
              className={cn(
                ...
                activeTab === 'balance' ? 'bg-white/20' : 'bg-muted-foreground/20',
              )}
            >
          ...
          >
            ...
            <span
              className={cn(
                'text-xs px-1.5 py-0.5 rounded-full hidden sm:inline',
                activeTab === 'dividend' ? 'bg-white/20' : 'bg-muted-foreground/20',
              )}
            >
          ...
          >
            ...
            <span
              className={cn(
                'text-xs px-1.5 py-0.5 rounded-full hidden sm:inline',
                activeTab === 'deposit' ? 'bg-white/20' : 'bg-muted-foreground/20',
              )}
            >
        ...
        {activeTab === 'balance' && accountBalances.length > 0 && (
          <div className="bg-gradient-to-br from-emerald-600/20 to-emerald-800/10 rounded-2xl p-5 border border-emerald-500/20">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-xs text-emerald-600/70 mb-1 font-bold">최신 계좌총액</p>
                <p className="text-2xl font-bold text-emerald-950">
                  {formatCurrency(latestBalance)}원
                </p>
              </div>
              <div className="text-right">
                <p className="text-xs text-emerald-600/70 mb-1 font-bold">기록된 개월 수</p>
        ...
        {activeTab === 'dividend' && dividendTransactions.length > 0 && (
          <div className="bg-gradient-to-br from-blue-600/20 to-blue-800/10 rounded-2xl p-5 border border-blue-500/20">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-xs text-blue-600/70 mb-1 font-bold">누적 배당금</p>
                <p className="text-2xl font-bold text-blue-950">
                  {formatCurrency(totalDividend)}원
                </p>
              </div>
              <div className="text-right">
                <p className="text-xs text-blue-600/70 mb-1 font-bold">올해 배당금</p>
        ...
        {activeTab === 'deposit' && depositTransactions.length > 0 && (
          <div className="bg-gradient-to-br from-purple-600/20 to-purple-800/10 rounded-2xl p-5 border border-purple-500/20">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-xs text-purple-600/70 mb-1 font-bold">총 입금액</p>
                <p className="text-2xl font-bold text-purple-950">
                  {formatCurrency(totalDeposit)}원
                </p>
              </div>
              <div className="text-right">
                <p className="text-xs text-purple-600/70 mb-1 font-bold">총 출금액</p>
                      ...
                      <div className="relative">
                        ...
                        <div className="absolute top-0 right-0 flex items-center gap-0.5">
                          ...
                          >
                            <Trash2
                              size={16}
                              className={deletingId === balance.id ? 'animate-pulse' : ''}
                            />
                          </button>
                        </div>
                        {/* 상단: 아이콘 + 날짜 + 설명 */}
          ...
          {(activeTab === 'dividend' || activeTab === 'deposit') &&
            (currentTransactions.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-20 text-center">
                ...
                <h3 className="text-lg font-medium text-foreground mb-2">
                  {activeTab === 'dividend' ? '배당내역이 없습니다' : '입출금내역이 없습니다'}
                </h3>
                <p className="text-sm text-muted-foreground max-w-[280px]">
                  {activeTab === 'dividend'
                      ...
                      <div className="relative">
                        ...
                        <div className="absolute top-0 right-0 flex items-center gap-0.5">
                          ...
                          >
                            <Trash2
                              size={16}
                              className={deletingId === tx.id ? 'animate-pulse' : ''}
                            />
                          </button>
                        </div>
                        {/* 상단: 아이콘 + 종목명 + 날짜 */}
