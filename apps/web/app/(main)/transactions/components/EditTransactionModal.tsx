...
import { type AccountBalanceRecord, updateAccountBalance } from '../../../actions/account-balance'
...
export function EditTransactionModal({
  ...
  useEffect(() => {
    ...
    if (editType === 'balance' && balanceData) {
      ...
      const dateStr = transactionData.trade_date.split('T')[0] || transactionData.trade_date
    ...
    try {
      ...
      if (editType === 'balance' && balanceData) {
        ...
        result = await updateDividend({
          originalDate: transactionData.trade_date.split('T')[0] || transactionData.trade_date,
          ...
          newAmountUSD: Number.parseFloat(dividendAmountUSD) || 0,
      ...
      if (result.success) {
        ...
        if (editType === 'balance' && balanceData) {
          ...
          queryClient.setQueryData<TransactionsResult>(queryKeys.transactions, (old) => {
      ...
      >
        ...
        <DialogHeader className="p-5 pb-0 flex-row items-center justify-between space-y-0">
          <div className="flex items-center gap-2">
            {getIcon()}
            <DialogTitle className="text-lg font-bold text-foreground">{getTitle()}</DialogTitle>
          </div>
          <button
            type="button"
            onClick={handleClose}
        ...
        <div className="p-5 space-y-5 max-h-[60vh] overflow-y-auto">
          ...
          {editType === 'balance' && (
            <>
              <div className="space-y-2">
                <span className="text-sm font-medium text-muted-foreground">연월</span>
                <YearMonthPicker
                  year={selectedYear}
                  month={selectedMonth}
              ...
              <div className="space-y-2">
                <span className="text-sm font-medium text-muted-foreground">계좌총액</span>
          ...
          {editType === 'dividend' && (
            <>
              ...
              />
              <div className="space-y-2">
                <span className="text-sm font-medium text-muted-foreground">배당금 (원화)</span>
                <div className="relative">
                  <Input
                    type="text"
                    inputMode="numeric"
                    value={dividendAmountKRW}
                    onChange={(e) => setDividendAmountKRW(formatNumber(e.target.value))}
                    onFocus={handleInputFocus}
                    placeholder="0"
              ...
              <div className="space-y-2">
                <span className="text-sm font-medium text-muted-foreground">계좌 유형</span>
                <Select value={dividendAccount} onValueChange={setDividendAccount}>
          ...
          {editType === 'deposit' && (
            <>
              ...
              <div className="space-y-2">
                <span className="text-sm font-medium text-muted-foreground">구분</span>
                <Select
                  value={depositType}
                  onValueChange={(val) => setDepositType(val as 'DEPOSIT' | 'WITHDRAW')}
                >
              ...
              <div className="space-y-2">
                <span className="text-sm font-medium text-muted-foreground">계좌</span>
              ...
              <div className="space-y-2">
                <span className="text-sm font-medium text-muted-foreground">금액</span>
                <div className="relative">
                  <Input
                    type="text"
                    inputMode="numeric"
                    value={depositAmount}
                    onChange={(e) => setDepositAmount(formatNumber(e.target.value))}
                    onFocus={handleInputFocus}
                    placeholder="0"
                    className="text-right pr-10"
                  />
              ...
              <div className="space-y-2">
                <span className="text-sm font-medium text-muted-foreground">메모</span>
                <Input
                  type="text"
                  value={depositMemo}
        ...
        <div className="p-5 pt-0 flex gap-2">
          ...
          >
            {isSubmitting ? <Loader2 className="w-4 h-4 animate-spin" /> : '수정'}
          </Button>
        </div>
      </DialogContent>
    </Dialog>
