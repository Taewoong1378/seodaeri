...
import type { YieldComparisonData, YieldComparisonDollarData } from '../../../../lib/google-sheets'
...
// 데이터 레이블 포맷
const formatLabel = (value: number) => `${value >= 0 ? '+' : ''}${value.toFixed(1)}%`
...
function SingleBarChart({
  ...
  return (
    <ResponsiveContainer width="100%" height={height as number | `${number}%`}>
      <BarChart data={chartData} margin={defaultMargin} barCategoryGap={isCramped ? '20%' : '25%'}>
        ...
        <Bar dataKey="value" radius={[4, 4, 0, 0]} maxBarSize={isCramped ? 36 : 48}>
          <LabelList dataKey="value" position="top" content={makeLabelRenderer()} />
          {chartData.map((entry) => (
            <Cell key={entry.name} fill={getBarColor(entry.name, entry.value)} />
          ))}
        </Bar>
      </BarChart>
    </ResponsiveContainer>
    ...
    <div className="flex items-center gap-0.5 bg-muted/50 rounded-lg p-0.5">
      <button
        ...
        className={`${px} ${py} ${text} font-medium rounded-md transition-all ${
          rateMode === 'basic' ? 'bg-background text-foreground shadow-sm' : 'text-muted-foreground'
        }`}
      >
        기본
      </button>
...
// ===== 누적수익률 비교 차트 =====
export function CumulativeYieldComparisonChart({ data, dollarData }: YieldComparisonChartProps) {
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="flex items-center justify-between">
        <h4 className="text-sm font-semibold text-foreground">누적수익률 비교</h4>
        <div className="flex items-center gap-2">
          {hasDollar && <RateModeToggle rateMode={rateMode} setRateMode={setRateMode} />}
          <ShareChartButton
          ...
          <LandscapeChartModal title={`누적수익률 비교 (${rateModeLabel})`}>
            <div className="flex flex-col w-full h-full">
              <div className="flex items-center justify-between mb-2 shrink-0">
                {hasDollar ? (
                  <RateModeToggle rateMode={rateMode} setRateMode={setRateMode} size="md" />
                ) : (
                  <div />
                )}
              </div>
...
// ===== 연평균 수익률 비교 차트 =====
export function AnnualizedYieldComparisonChart({ data, dollarData }: YieldComparisonChartProps) {
  const hiddenChartRef = useRef<HTMLDivElement>(null)
  const hasDollar = !!dollarData
  const [rateMode, setRateMode] = useState<RateMode>('basic')
  ...
  return (
    <div className="space-y-3">
      {/* Header */}
      <div className="flex items-center justify-between">
        <h4 className="text-sm font-semibold text-foreground">연평균 수익률 비교</h4>
        <div className="flex items-center gap-2">
          {hasDollar && <RateModeToggle rateMode={rateMode} setRateMode={setRateMode} />}
          <ShareChartButton
            chartRef={hiddenChartRef}
...
export function YieldComparisonChart({ data, dollarData }: YieldComparisonChartProps) {
  return (
