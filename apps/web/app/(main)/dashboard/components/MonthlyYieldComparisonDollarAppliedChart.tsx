...
export function MonthlyYieldComparisonDollarAppliedChart({
  data,
}: MonthlyYieldComparisonDollarAppliedChartProps) {
    ...
    <div ref={chartRef} className="space-y-4">
      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between">
          <h4 className="text-sm font-semibold text-foreground">
            {currentYear}년 수익률 현황 (환율 반영)
          </h4>
          <div className="flex items-center gap-2">
            <ShareChartButton
              chartRef={chartRef}
              title={`${currentYear}년 수익률 현황 (환율 반영)`}
            />
                ...
                <div className="flex items-center justify-center gap-6 mb-4 shrink-0">
                  <div className="flex items-center gap-2">
                    <div
                      ...
                      style={{ backgroundColor: COLORS.currentMonth }}
                    />
                  ...
                  <ResponsiveContainer width="100%" height="100%">
                    ...
                    >
                      ...
                      <Bar dataKey="currentMonth" fill={COLORS.currentMonth} radius={[4, 4, 0, 0]}>
                        {chartData.map((entry) => (
                          <Cell
                            key={`currentMonth-${entry.name}`}
                            fill={getBarColor(entry.currentMonth, COLORS.currentMonth)}
                          />
                        ))}
                      </Bar>
      ...
      <div className="flex gap-2 overflow-x-auto pb-1 -mx-1 px-1 scrollbar-hide">
        {chartData.map((item) => (
          ...
          >
            <span className="text-[9px] text-muted-foreground block mb-0.5 truncate">
              {item.name}
            </span>
            <div className="space-y-0">
              <div
                className={`text-[11px] font-semibold leading-tight ${item.thisYear >= 0 ? 'text-orange-500' : 'text-muted-foreground'}`}
              >
                {item.thisYear >= 0 ? '+' : ''}
                {item.thisYear.toFixed(1)}%
              </div>
              <div
                className={`text-[9px] leading-tight ${item.currentMonth >= 0 ? 'text-gray-500' : 'text-muted-foreground'}`}
              >
                {data.currentMonth} {item.currentMonth >= 0 ? '+' : ''}
