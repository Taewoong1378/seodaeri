  ...
  if (active && payload && payload.length) {
    return (
      <div className="bg-popover border border-border px-3 py-2 rounded-lg shadow-xl">
        ...
        <p className="text-foreground text-sm font-bold">{payload[0].value.toLocaleString()}원</p>
      </div>
    )
  }
  return null
  ...
  const renderChart = (height = '220px', showYAxis = false) => (
    <div className="relative w-full" style={{ height }}>
      <ResponsiveContainer width="100%" height="100%">
        <BarChart
          data={fullYearData}
          margin={{ top: 10, right: showYAxis ? 10 : 0, left: showYAxis ? 0 : 0, bottom: 20 }}
        >
          <defs>
          ...
          >
            {fullYearData.map((entry) => (
              <Cell
                ...
                style={{
                  filter:
                    entry.amount > 0 ? 'drop-shadow(0 4px 6px rgba(59, 130, 246, 0.15))' : 'none',
                }}
              />
            ))}
          </Bar>
            ...
            <div className="flex flex-col h-full">
              <div className="flex items-center justify-center mb-4 shrink-0">
                <p className="text-lg text-muted-foreground">
                  총{' '}
                  <span className="text-foreground font-bold">{yearTotal.toLocaleString()}원</span>
                </p>
              </div>
              <div className="flex-1 min-h-0">{renderChart('100%', true)}</div>
            </div>
          </LandscapeChartModal>
        </div>
      </div>
        ...
        >
          <div>
            <h3 style={{ fontSize: '20px', fontWeight: 700, color: '#1e293b', margin: 0 }}>
              {selectedYear}년 월별 배당금
            </h3>
            <p style={{ fontSize: '14px', color: '#64748b', margin: '4px 0 0 0' }}>
              총 {yearTotal.toLocaleString()}원
            </p>
          </div>
        </div>
        <div style={{ width: '100%', height: '350px' }}>{renderChart('100%', true)}</div>
      </div>
    </div>
  )
}
