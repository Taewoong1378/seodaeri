...
function StatItem({
  ...
  amount,
  yield: yieldPercent,
}: { label: string; amount: number; yield?: number }) {
  ...
  return (
    <div className="space-y-1.5">
      ...
      {label === '원금' ? (
        <p className="text-base font-semibold text-foreground">{amount.toLocaleString()}원</p>
      ) : (
...
export function HeroCard({
  ...
  const hasAnyGoal =
    (finalAssetGoal && finalAssetGoal > 0) || (annualDepositGoal && annualDepositGoal > 0)
      ...
      <div className="relative z-10 p-6 space-y-6">
        ...
        <div className="space-y-3">
          {hasAnyGoal ? (
            <>
              {/* 연간 입금액 목표 달성률 */}
              {annualDepositGoal && annualDepositGoal > 0 && (
                <div className="space-y-1.5">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-1.5">
                      ...
                      <button
                        type="button"
                        onClick={onEditAnnualDepositGoal}
                  ...
                  <div className="w-full h-2 bg-gray-100 rounded-full overflow-hidden">
                    <div
                      ...
                      style={{
                        width: `${Math.min((thisYearDeposit / annualDepositGoal) * 100, 100)}%`,
                      }}
                    />
                  </div>
                  <p className="text-[11px] text-muted-foreground">
                    목표 {annualDepositGoal.toLocaleString()}원
                  </p>
                </div>
              )}

              {/* 최종 총자산 목표 달성률 */}
              {finalAssetGoal && finalAssetGoal > 0 && (
                <div className="space-y-1.5">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-1.5">
                      <span className="text-xs font-medium text-muted-foreground">
                    ...
                    <span className="text-xs font-bold text-blue-600">
                      {Math.min((totalAsset / finalAssetGoal) * 100, 100).toFixed(1)}%
                    </span>
                  </div>
                  <div className="w-full h-2 bg-gray-100 rounded-full overflow-hidden">
                    <div
                      className="h-full bg-blue-600 rounded-full transition-all duration-500"
                      style={{ width: `${Math.min((totalAsset / finalAssetGoal) * 100, 100)}%` }}
                    />
                  </div>
                  <p className="text-[11px] text-muted-foreground">
                    목표 {finalAssetGoal.toLocaleString()}원
                  </p>
                </div>
              )}
        ...
        {investmentDays > 0 && (
          <div className="flex justify-center pt-2">
            <p className="text-sm font-medium text-muted-foreground text-[20px]">
              벌써{' '}
              <span className="text-blue-600 font-bold">{investmentDays.toLocaleString()}</span>일째
              투자중이에요!
            </p>
          </div>
        )}
