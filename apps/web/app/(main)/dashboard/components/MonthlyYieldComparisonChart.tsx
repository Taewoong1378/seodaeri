    ...
    <div ref={chartRef} className="space-y-4">
      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between">
          <h4 className="text-sm font-semibold text-foreground">
            {currentYear}년 {data.currentMonth}, 누적 수익률 현황
          </h4>
          <div className="flex items-center gap-2">
            <ShareChartButton
              chartRef={hiddenChartRef}
              title={`${currentYear}년 ${data.currentMonth}, 누적 수익률 현황`}
            />
      ...
      <div className="flex gap-1.5 overflow-x-auto pb-1 -mx-1 px-1 scrollbar-hide">
        {chartData.map((item) => (
          ...
          >
            <span className="text-[8px] text-muted-foreground block mb-0.5 truncate">
              {item.name}
            </span>
            <div className="space-y-0">
              <div
                className={`text-[10px] font-semibold leading-tight ${item.thisYear >= 0 ? 'text-orange-500' : 'text-muted-foreground'}`}
              >
                {item.thisYear >= 0 ? '+' : ''}
                {item.thisYear.toFixed(1)}%
              </div>
              <div
                className={`text-[8px] leading-tight ${item.currentMonth >= 0 ? 'text-gray-500' : 'text-muted-foreground'}`}
              >
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
            <div
              style={{
                ...
                height: '16px',
                borderRadius: '2px',
                backgroundColor: COLORS.thisYear,
              }}
            />
