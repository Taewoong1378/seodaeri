...
export function PerformanceComparisonChart({ data }: PerformanceComparisonChartProps) {
  ...
  const hasDollarData = data.some(
    (d) => d.sp500Dollar !== undefined || d.nasdaqDollar !== undefined,
  )
  ...
  return (
    <div ref={chartRef} className="space-y-2">
              ...
              <div className="flex items-center justify-between mb-4">
                <div className="flex flex-wrap items-center gap-x-6 gap-y-2">
                  {LINES.map((line) => {
                    const label =
                      rateMode === 'dollar' &&
                      (line.dataKey === 'sp500' || line.dataKey === 'nasdaq')
                        ? `${line.name}(환율)`
                        : line.name
                    return (
      ...
      <div className="h-[250px]">
        <ResponsiveContainer width="100%" height="100%">
          <LineChart data={chartDisplayData} margin={{ top: 10, right: 10, left: 10, bottom: 5 }}>
            <CartesianGrid strokeDasharray="3 3" stroke="#e2e8f0" vertical={false} />
            <XAxis
              dataKey="date"
              axisLine={{ stroke: '#cbd5e1' }}
