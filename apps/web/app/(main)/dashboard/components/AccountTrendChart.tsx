...
export function AccountTrendChart({
...
}: AccountTrendChartProps): ReactElement {
  ...
  const extendedData = filteredData.map((item) => {
    ...
    if (
      ...
      // 마지막 유효한 누적입금액 찾기
      const lastValidDeposit =
        filteredData.filter((d) => d.date < currentDateStr && d.cumulativeDeposit > 0).pop()
          ?.cumulativeDeposit ||
        currentTotalInvested ||
        0
  ...
  if (currentTotalAsset && currentTotalAsset > 0) {
    ...
    if (!hasCurrentMonth) {
      const lastValidDeposit =
        filteredData.filter((d) => d.cumulativeDeposit > 0).pop()?.cumulativeDeposit ||
        currentTotalInvested ||
        0
          ...
          <LandscapeChartModal title="월별 계좌추세">
            <div className="flex items-center justify-center gap-6 mb-4">
              ...
              <div className="flex items-center gap-2">
                <div
                  className="w-4 h-4 rounded-sm"
                  style={{ backgroundColor: 'rgba(163, 230, 53, 0.7)' }}
                />
