...
export function DashboardContent() {
  ...
  const [goalModalType, setGoalModalType] = useState<'finalAsset' | 'annualDeposit'>('finalAsset')
  const [isGoalModalOpen, setIsGoalModalOpen] = useState(false)
  const [dividendAccountFilter, setDividendAccountFilter] = useState<
    'all' | 'general' | 'taxSaving'
  >('all')
  ...
  useEffect(() => {
    if (!isLoading && data && typeof window !== 'undefined') {
      ;(window as any).ReactNativeWebView?.postMessage(JSON.stringify({ type: 'App.Ready' }))
    }
  }, [isLoading, data])
  ...
  return (
    <div className="space-y-6">
      ...
      <section className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-700">
        {/* 배너 캐러셀 */}
        {carouselBanners.length > 0 && <SmallBannerCarousel items={carouselBanners} />}
          ...
          (() => {
            ...
            const thisYearProfit =
              displayData.monthlyProfitLoss.length > 0
                ? displayData.monthlyProfitLoss.reduce((sum, m) => sum + m.profit - m.loss, 0)
                : undefined
            // 시트에서 읽은 수익률 우선, 없으면 클라이언트 계산 fallback
            const thisYearYield =
                    ...
                    <CardContent className="p-4">
                      <div className="flex flex-col items-center justify-center py-8 text-center">
                        ...
                        <p className="text-sm text-muted-foreground mb-6 max-w-[280px]">
                          내역 페이지에서 계좌총액, 배당금, 입출금 내역을 기록해보세요
                        </p>
          ...
          dividend: (
            <div className="space-y-6">
              ...
              {activeDividendData?.yearlyDividendSummary && (
                <Card className="border-border bg-card shadow-sm rounded-[24px] overflow-hidden">
                  <CardContent className="p-4">
                    <YearlyDividendChart data={activeDividendData.yearlyDividendSummary} />
                  </CardContent>
                </Card>
              )}
              ...
              {activeDividendData?.rollingAverageDividend && (
                <Card className="border-border bg-card shadow-sm rounded-[24px] overflow-hidden">
                  <CardContent className="px-4 pt-4 pb-2">
                    <RollingAverageDividendChart data={activeDividendData.rollingAverageDividend} />
                  </CardContent>
                </Card>
              )}
              ...
              {activeDividendData?.cumulativeDividend && (
                <Card className="border-border bg-card shadow-sm rounded-[24px] overflow-hidden">
                  <CardContent className="px-4 pt-4 pb-2">
                    <CumulativeDividendChart data={activeDividendData.cumulativeDividend} />
                  </CardContent>
                </Card>
              )}
