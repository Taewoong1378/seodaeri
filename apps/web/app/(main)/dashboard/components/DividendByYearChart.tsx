...
const getYearColor = (year: number, variant: 'default' | 'landing' = 'default'): string => {
  if (variant === 'landing') {
    ...
    const years = Object.keys(LANDING_YEAR_COLORS)
      .map(Number)
      .sort((a, b) => a - b)
...
export function DividendByYearChart({ data, variant = 'default' }: DividendByYearChartProps) {
  ...
  const CustomLegend = ({ isModal = false }: { isModal?: boolean }) => (
    <div
      className={`flex flex-wrap items-center ${isModal ? 'justify-center gap-4' : 'justify-end gap-2'}`}
    >
      {data.years.map((year) => (
        <div key={year} className="flex items-center gap-1">
          ...
          />
          <span className={`${isModal ? 'text-sm' : 'text-[10px]'} text-muted-foreground`}>
            {year}
          </span>
        </div>
      ))}
