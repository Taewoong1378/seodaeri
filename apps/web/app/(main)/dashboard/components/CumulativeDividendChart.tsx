...
export function CumulativeDividendChart({ data }: CumulativeDividendChartProps): ReactElement {
  ...
  // 스크롤을 우측 끝(최신)으로 이동
  // biome-ignore lint/correctness/useExhaustiveDependencies: scroll to end when data length changes
  useEffect(() => {
  ...
  const renderChart = (isModal = false) => (
    ...
    >
      ...
      <XAxis
        ...
        {...(isModal
          ? modalXTicks !== undefined
            ? { ticks: modalXTicks as string[], interval: 0 as const }
            : { interval: 0 as const }
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-muted-foreground mt-0.5">
            총 누적: {formatCurrency(currentCumulative)}
          </p>
        </div>
