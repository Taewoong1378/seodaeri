...
export function HoldingInputModal({
  ...
  const handleCashKrwChange = useCallback((e: React.ChangeEvent<HTMLInputElement>) => {
    setCashKrw(formatNumber(e.target.value))
    setError(null)
  }, [])

  const handleCashUsdChange = useCallback((e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      try {
        ...
        if (result.success) {
          ...
          queryClient.setQueryData<DashboardData | null>(queryKeys.dashboard, (old) => {
            if (!old) return old
            const portfolio = old.portfolio.filter((p) => p.ticker !== CASH_TICKER)
            if (krwAmount > 0 || usdAmount > 0) {
              portfolio.push({
                ticker: CASH_TICKER,
                name: '현금',
          ...
          if (existing) {
            ...
            newPortfolio = [
              ...old.portfolio,
              {
                ...
                yieldPercent: 0,
                weight: 0,
                rowIndex: old.portfolio.length + 9,
              },
            ]
          }
          ...
          {isCash ? (
            ...
            <div className="space-y-3">
              <div className="space-y-2">
                <label htmlFor="cashKrw" className="text-sm font-medium text-muted-foreground">
                  원화 (₩)
                </label>
                <Input
                  type="text"
              ...
              <div className="space-y-2">
                <label htmlFor="cashUsd" className="text-sm font-medium text-muted-foreground">
                  달러 ($)
                </label>
                <Input
                  type="text"
            ...
            <>
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div className="space-y-2">
                  <label htmlFor="avgPrice" className="text-sm font-medium text-muted-foreground">
                    평단가 ({currency === 'USD' ? '$' : '₩'})
                  </label>
                  <Input
                    type="text"
