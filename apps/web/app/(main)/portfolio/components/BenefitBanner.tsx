...
export function BenefitBanner() {
  ...
  return (
    <Link href={bannerData.link} target="_blank" onClick={handleTrackClick} className="block">
      <div className="relative overflow-hidden rounded-2xl bg-card border border-border shadow-sm">
        <div className="relative p-4 flex items-center gap-4">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-sm font-semibold text-foreground truncate">{bannerData.title}</p>
            <p className="text-[11px] text-muted-foreground truncate">{bannerData.subtitle}</p>
          </div>

          {/* 화살표 버튼 */}
          <div
            className={`w-9 h-9 rounded-full ${bannerData.buttonBg} flex items-center justify-center shadow-md shrink-0`}
          >
