 ...
 */
export async function checkSheetConnection(
  existingSession?: { user?: { id?: string; email?: string | null } } | null,
): Promise<{
  connected: boolean
  sheetId?: string
  userExists: boolean
...
export async function createNewSheet(): Promise<OnboardingResult> {
  ...
  try {
    ...
    if (error.code === 403) {
      return {
        success: false,
        error: '템플릿에 접근 권한이 없습니다. 템플릿이 공유되어 있는지 확인하세요.',
      }
    }
...
export async function connectSheetById(sheetId: string): Promise<OnboardingResult> {
  const session = await auth()
  console.log(
    '[connectSheetById] session.user.id:',
    session?.user?.id,
    'email:',
    session?.user?.email,
  )
