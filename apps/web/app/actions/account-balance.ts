...
export async function saveAccountBalance(
  ...
  try {
    ...
    if (!user.spreadsheet_id) {
      ...
      const { error: dbError } = await (supabase as any).from('account_balances').upsert(
        {
          user_id: dbUserId,
...
export async function deleteAccountBalance(
  ...
  console.log('[deleteAccountBalance] Called with input:', JSON.stringify(input))
  ...
  try {
    ...
    console.log('[deleteAccountBalance] User found, spreadsheet_id:', user.spreadsheet_id)
    ...
    if (rows && rows.length > 1) {
      ...
      console.log('[deleteAccountBalance] Looking for year:', inputYear, 'month:', inputMonthNum)
      ...
      for (let i = 1; i < rows.length; i++) {
        ...
        if (
          ...
          console.log(`[deleteAccountBalance] Match found at row ${i}, clearing H column...`)
  ...
  try {
    ...
    if (!user.spreadsheet_id) {
      ...
      if (input.originalYearMonth !== input.newYearMonth) {
        ...
        const { error: insertError } = await (supabase as any).from('account_balances').upsert(
          {
            user_id: userId,
    ...
    if (input.originalYearMonth !== input.newYearMonth) {
      ...
      await (supabase as any).from('account_balances').upsert(
        {
          user_id: userId,
          year_month: input.newYearMonth,
          balance: input.newBalance,
  ...
  try {
    ...
    if (allRows && allRows.length > 0) {
      ...
      for (let i = 44; i < Math.min(55, allRows.length); i++) {
        if (allRows[i]?.some((cell: any) => cell !== '' && cell !== null && cell !== undefined)) {
