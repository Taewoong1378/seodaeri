      ...
      messages: [
        ...
        },
        {
          ...
          content: [
            {
              type: 'text',
              text: 'Extract ALL trade entries from this image. Return them as a JSON array.',
            },
            {
              type: 'image_url',
              image_url: {
...
export async function saveTradeTransactions(trades: TradeInput[]): Promise<SaveTradeResult> {
  ...
  try {
    ...
    console.log(
      '[saveTradeTransactions] Insert result - error:',
      insertError,
      'data:',
      insertedData,
    )
...
export async function deleteTransaction(input: DeleteTransactionInput): Promise<SaveTradeResult> {
  ...
  try {
    ...
    if (input.id) {
      ...
      console.log(
        '[deleteTransaction] Supabase delete by conditions - error:',
        dbError,
        'count:',
        count,
      )
      ...
      try {
        ...
        if (rows) {
          ...
          for (let i = 1; i < rows.length; i++) {
            ...
            const matchType = (input.type === 'BUY' && isBuy) || (input.type === 'SELL' && !isBuy)

            // 처음 몇 개 행만 상세 로깅
            if (i <= 5) {
              console.log(
                `[deleteTransaction] Row ${i} (offset=${offset}): date=${normalizedRowDate}, ticker=${rowTicker}, type=${rowType}, price=${rowPrice}, qty=${rowQuantity}`,
              )
              console.log(
                `[deleteTransaction] Input: date=${input.date}, ticker=${input.ticker}, type=${input.type}, price=${input.price}, qty=${input.quantity}`,
              )
            }
