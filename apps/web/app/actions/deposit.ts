  ...
  try {
    ...
    const rowData = [
      ...
      input.type === 'DEPOSIT'
        ...
        : `-₩${input.amount.toLocaleString()}`, // G: 금액
      input.memo || '', // H: 비고
    ]

    // 기존 데이터 마지막 행 다음에 추가 (append API가 자동으로 마지막 행 감지)
    await appendSheetData(session.accessToken, user.spreadsheet_id, "'6. 입금내역'!A:H", [rowData])
...
export async function saveAutoDepositSetting(
  setting: AutoDepositSetting,
...
function getDefaultAccounts(): string[] {
  return ['일반계좌1', '일반계좌2', '개인연금1', '개인연금2', 'IRP 1', 'IRP 2', 'ISA', '퇴직연금DC']
}
  ...
  try {
    ...
    if (rows && rows.length > 1) {
      ...
      for (let i = 1; i < rows.length; i++) {
        ...
        const matchType =
          (input.type === 'DEPOSIT' && isDeposit) || (input.type === 'WITHDRAW' && !isDeposit)

        // 처음 몇 개 행만 상세 로깅
        if (i <= 5) {
          console.log(
            `[deleteDeposit] Row ${i}: date=${rowDate}, type=${rowType}, amount=${rowAmount}, isDeposit=${isDeposit}`,
          )
          console.log(
            `[deleteDeposit] Input: date=${input.date}, type=${input.type}, amount=${input.amount}`,
          )
        }

        if (rowDate === input.date && matchType && Math.abs(rowAmount - input.amount) < 1) {
  ...
  try {
    ...
    if (rows && rows.length > 1) {
      ...
      for (let i = 1; i < rows.length; i++) {
        ...
        const matchType =
          (input.originalType === 'DEPOSIT' && isDeposit) ||
        ...
        if (
          ...
          const newRowData = [
            ...
            input.newType === 'DEPOSIT'
              ? `₩${input.newAmount.toLocaleString()}`
              : `-₩${input.newAmount.toLocaleString()}`,
            input.newMemo || '',
          ]
