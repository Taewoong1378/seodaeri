...
export async function getDashboardData(): Promise<DashboardData | null> {
  ...
  try {
    ...
    // 배당 데이터 파싱
    const dividends: DividendRecord[] = dividendRows ? parseDividendData(dividendRows) : []
    ...
    const dividendByAccount =
      generalDividends.length > 0 || taxSavingDividends.length > 0
        ? {
            general: computeDividendAccountData(generalDividends, accountTrendStartMonth),
            taxSaving: computeDividendAccountData(taxSavingDividends, accountTrendStartMonth),
          }
        : undefined
    ...
    if (missingPriceItems.length > 0) {
      try {
        ...
        console.log(
          `[Dashboard] Sheet missing prices for ${missingTickers.length} items:`,
          missingTickers.join(', '),
        )
        ...
        for (const item of missingPriceItems) {
          ...
          if (kisPrice && kisPrice.price > 0) {
            ...
            const priceKRW =
              kisPrice.currency === 'USD' ? kisPrice.price * currentRate : kisPrice.price
            item.currentPrice = Math.round(priceKRW)
            item.totalValue = Math.round(priceKRW * item.quantity)
            const invested = item.avgPrice * item.quantity
            item.profit = item.totalValue - invested
            item.yieldPercent =
              invested > 0 ? Number(((item.profit / invested) * 100).toFixed(2)) : 0
            console.log(
              `[Dashboard] KIS price override: ${item.ticker} = ${kisPrice.price} ${kisPrice.currency} → ${item.currentPrice} KRW (yield: ${item.yieldPercent}%)`,
            )
          }
        }
      } catch (error) {
    ...
    ;(async () => {
      try {
        ...
        if (portfolio.length > 0) {
          ...
          await supabase.from('holdings').upsert(holdingsData, { onConflict: 'user_id,ticker' })
    ...
    const monthlyYieldComparisonDollarApplied: MonthlyYieldComparisonDollarAppliedData | null =
      enrichedRows ? parseMonthlyYieldComparisonDollarApplied(enrichedRows) : null
...
export async function syncPortfolio() {
  ...
  if (portfolio.length > 0) {
    ...
    const cacheData = portfolio.map((item) => ({
      ...
      quantity: item.quantity,
      current_price:
        item.currency === 'USD' && syncExchangeRate > 0
    ...
    await supabase.from('portfolio_cache').upsert(cacheData, { onConflict: 'user_id,ticker' })
    ...
    const { data: existingHoldings } = await supabase
      ...
      .eq('user_id', userId)
    const existingAvgPriceMap = new Map(existingHoldings?.map((h) => [h.ticker, h.avg_price]) || [])
...
async function getStandaloneDashboardData(userId: string): Promise<DashboardData> {
  ...
  try {
    ...
    const dividendByAccount =
      generalDividends.length > 0 || taxSavingDividends.length > 0
        ? {
            general: computeDividendAccountData(generalDividends, standaloneStartMonth),
            taxSaving: computeDividendAccountData(taxSavingDividends, standaloneStartMonth),
          }
        : undefined

    // 4. 주요 지수 수익률 비교 데이터 조회
    const majorIndexYieldComparison = await provider.getMajorIndexYieldComparison(
      userId,
      historicalRates,
      marketData,
    )
    ...
    if (majorIndexYieldComparison && marketData) {
      ...
      const prevKey =
        cm === 1 ? `${prevYearStr}.12` : `${yearStr}.${String(cm - 1).padStart(2, '0')}`
    ...
    if (majorIndexYieldComparison && marketData) {
      ...
      const prevKey2 =
        cm2 === 1 ? `${prevYearStr2}.12` : `${yearStr2}.${String(cm2 - 1).padStart(2, '0')}`
      ...
      for (let m = 1; m <= currentMonth; m++) {
        ...
        const portfolioReturn =
          trend && trend.cumulativeDeposit > 0
            ? round1(
                ((trend.totalAccount - trend.cumulativeDeposit) / trend.cumulativeDeposit) * 100,
              )
            : 0
