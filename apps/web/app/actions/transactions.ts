...
export async function getTransactions(): Promise<TransactionsResult> {
  ...
  if (session.isDemo) {
    ...
    // 날짜 기준 정렬하여 반환
    const allTransactions = [...dividendTransactions, ...depositTransactions].sort(
  ...
  try {
    ...
    // Sheet 모드: Google Sheets에서 입금내역, 배당내역 조회
      ...
      try {
        ...
        const [depositRows, dividendRows] = await Promise.all([
          fetchSheetData(session.accessToken, user.spreadsheet_id, "'6. 입금내역'!A:H").catch(
            (e) => {
              console.error('[Sheet] 6. 입금내역 읽기 실패:', e)
              return null
            },
          ),
          fetchSheetData(session.accessToken, user.spreadsheet_id, "'7. 배당내역'!A:K").catch(
            (e) => {
    ...
    const allTransactions = [...appTransactions, ...additionalTransactions].sort((a, b) => {
