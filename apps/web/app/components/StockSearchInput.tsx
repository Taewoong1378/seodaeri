...
export function StockSearchInput({
      ...
      <div className="space-y-2">
        ...
        <div
          className={`flex items-center gap-3 p-3 h-11 rounded-lg border overflow-hidden transition-all duration-200 ${
            darkTheme ? 'bg-white/5 border-white/10' : 'bg-background border-border shadow-sm'
          }`}
        >
  ...
  return (
    <div className="space-y-2 relative">
      <label
        htmlFor="stock-search"
        className={`text-sm font-medium ${darkTheme ? 'text-slate-300' : 'text-muted-foreground'}`}
      >
        {label}
      </label>
        ...
        >
          ...
          {isSearching && (
            <div className="mr-4 flex-shrink-0">
              <Loader2
                size={18}
                className={`animate-spin ${darkTheme ? 'text-slate-500' : 'text-primary'}`}
              />
            </div>
          )}
        </div>
        ...
        {showResults && results.length > 0 && (
          <div
            className={`absolute z-50 w-full mt-2 rounded-lg shadow-xl max-h-48 overflow-y-auto overflow-x-hidden overscroll-contain animate-in fade-in zoom-in-95 duration-200 ${
              darkTheme ? 'bg-[#1e293b] border border-white/10' : 'bg-popover border border-border'
            }`}
          >
            {results.map((stock) => (
              <button
                ...
                <div className="min-w-0 flex-1 mr-3">
                  <div
                    className={`font-medium truncate ${
                      darkTheme ? 'text-white' : 'text-foreground group-hover/item:text-primary'
                    }`}
                  >
                    {stock.name}
                  </div>
      ...
      {!selectedCode && query && !isSearching && results.length === 0 && (
        <p className={`text-xs px-1 ${darkTheme ? 'text-slate-500' : 'text-muted-foreground'}`}>
