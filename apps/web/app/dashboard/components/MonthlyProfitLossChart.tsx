...
export function MonthlyProfitLossChart({ data }: MonthlyProfitLossChartProps) {
  ...
  return (
    <div ref={chartRef} className="space-y-4">
      ...
      >
        <div className="mb-8">
          <h3 className="text-2xl font-bold text-white mb-1">월별 손익</h3>
          <p className="text-sm text-slate-400">
            순손익 <span className={netTotal >= 0 ? 'text-orange-400' : 'text-slate-400'}>{netTotal >= 0 ? '+' : ''}{netTotal.toLocaleString()}원</span>
          </p>
        </div>
        ...
        <div className="w-full h-[500px] mb-8">
          <ResponsiveContainer width="100%" height="100%">
            ...
            >
              <defs>
                <linearGradient id="profitGradientPLHidden" x1="0" y1="0" x2="0" y2="1">
                  <stop offset="0%" stopColor="#f97316" stopOpacity={1} />
                  <stop offset="100%" stopColor="#ea580c" stopOpacity={0.8} />
                </linearGradient>
                <linearGradient id="lossGradientPLHidden" x1="0" y1="1" x2="0" y2="0">
                  <stop offset="0%" stopColor="#475569" stopOpacity={1} />
                  <stop offset="100%" stopColor="#334155" stopOpacity={0.8} />
                </linearGradient>
              </defs>
              ...
              >
                {chartData.map((entry, index) => (
                  <Cell
                    key={`profit-${entry.month}`}
                    fill={entry.profit > 0 ? 'url(#profitGradientPLHidden)' : 'transparent'}
                  />
                ))}
              </Bar>
              <Bar
              ...
              >
                {chartData.map((entry, index) => (
                  <Cell
                    key={`loss-${entry.month}`}
                    fill={entry.lossNegative < 0 ? 'url(#lossGradientPLHidden)' : 'transparent'}
                  />
                ))}
              </Bar>
            </BarChart>
        ...
        <div className="grid grid-cols-2 gap-4">
          <div className="flex flex-col p-4 rounded-xl bg-white/5 border border-white/5">
            <div className="flex items-center gap-2 mb-1">
              ...
              <span className="text-xs text-slate-400">총 수익</span>
            </div>
          ...
          <div className="flex flex-col p-4 rounded-xl bg-white/5 border border-white/5">
            <div className="flex items-center gap-2 mb-1">
              ...
              <span className="text-xs text-slate-400">총 손실</span>
            </div>
