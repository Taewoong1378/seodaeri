...
async function fetchAlternativeAssetCSV(): Promise<AlternativeAsset[]> {
  ...
  console.log(
    `[AlternativeAsset] CSV length: ${csvText.length}, first 200 chars: ${csvText.substring(0, 200)}`,
  )

  const assets = parseAlternativeAssetCSV(csvText)

  console.log(
    `[AlternativeAsset] Parsed ${assets.length} assets:`,
    assets.map((a) => `${a.code}=${a.price}`).join(', '),
  )
  return assets
}
...
export function searchAlternativeAssets(query: string): AlternativeAsset[] {
  ...
  const matched = ASSET_DEFINITIONS.filter(
    (def) =>
