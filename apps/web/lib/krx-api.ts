...
/**
 * 가장 최근 영업일 계산 (주말 제외, 오전 9시 이전이면 전일)
 */
function getLastBusinessDate(): Date {
  const now = new Date();
  const date = new Date(now);

  // 오전 9시 이전이면 전일 데이터 사용 (장 시작 전)
  if (now.getHours() < 9) {
...
async function fetchKRX<T>(endpoint: string, basDd: string): Promise<T[]> {
  ...
  if (!apiKey) {
    ...
    throw new Error('KRX API Key가 설정되지 않았습니다. 환경변수를 확인해주세요.');
  }
  ...
  if (!response.ok) {
    console.error('[KRX API] HTTP Error:', response.status, response.statusText);
  ...
  if (data.respCode && data.respCode !== '200') {
    ...
    if (data.respCode === '401') {
      throw new Error('KRX API 인증 실패: API KEY가 승인되지 않았거나 해당 API 이용신청이 필요합니다.');
    }
