  ...
  for (const file of files) {
    try {
      ...
      for (const row of rows) {
        ...
        allStocks.push({
          code: symbol,
          name: name.replace(' Common Stock', '').replace(' Class A', '').trim(),
          market: file.market,
  ...
  try {
    ...
    do {
      ...
      if (offset === 0) {
        ...
        console.log(
          `[downloadCSV] ETF: total ${totalRecords} records, fetching in pages of ${pageSize}...`,
        )
      }
 ...
 */
export async function searchUSStocksLocal(query: string, limit = 20): Promise<USStock[]> {
  ...
  return stocks
    .filter((stock) => stock.code.includes(q) || stock.name.toUpperCase().includes(q))
    .slice(0, limit)
}
